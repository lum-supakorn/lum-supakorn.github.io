...
We address this by introducing an auxiliary point $$k'$$. In fact, the $$\phi_{k'}$$ introduced earlier is the $$\phi_k$$ defined on this auxiliary point. It is the point which lies on the segment joining the straddling cell centers. We want this point to be as close to $$k$$ as possible to minimize the approximation error to be introduced after this. The shortest $$(\textbf{r}_k - \textbf{r}_k')$$ occurs when it is orthogonal to $$(\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}})$$, so the interpolation factor is calculated by projecting the displacement vector $$\textbf{r}_k - \textbf{r}_{\mathrm{P}}$$ onto $$\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}}$$ and get the relative magnitude:

$$
\xi_k = \frac{(\textbf{r}_k - \textbf{r}_{\mathrm{P}}) \cdot (\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}})}{|\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}}|^2}
$$

and the position of the auxiliary point, $$\textbf{r}_{k'}$$ is obtained by scaling $$\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}}$$ by $$\xi_k$$:

$$
\textbf{r}_{k'} = \textbf{r}_{\mathrm{P}} + \xi_k(\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}}) = (1-\xi_k)\textbf{r}_{\mathrm{P}} + \xi_k\textbf{r}_{\mathrm{N}_k}
$$
...
Now that we have $$\phi_{k'}$$, we need to approximate $$\phi_k$$. We do this by calculating the directional difference with the gradient field $$\nabla \phi$$:

$$
\phi_k = \phi_{k'} + (\nabla \phi)_{k'} \cdot (\textbf{r}_k-\textbf{r}_{k'})
$$

The second term describes the change in $$\phi_k$$ as we move from $$\textbf{r}_{k'}$$ to $$\textbf{r}_k$$. This term will introduce an additional approximation error, and that is why we need $$(\textbf{r}_k-\textbf{r}_{k'})$$ to be small. If information on $$\nabla \phi$$ is available at the center of each cell, $$(\nabla \phi)_{k'}$$ can be obtained with linear interpolation in the same manner as $$\phi_{k'}$$. Since $$\nabla \phi$$ depends on $$\phi$$ on each cell and vice versa, an iterative method is needed to obtain both. 
...
$$
\frac{\partial\phi_k}{\partial \textbf{n}} \approx \frac{\phi_{\mathrm{N}_k}-\phi_{\mathrm{P}}}{|\textbf{r}_{\mathrm{N}_k} - \textbf{r}_{\mathrm{P}}|}
$$
...
$$
\frac{\partial\phi_k}{\partial \textbf{n}} \approx \frac{\phi_{\mathrm{N}_k}-\phi_{\mathrm{P}}}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|} + \frac{(\nabla \phi)_{\mathrm{N}_k} \cdot (\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{N}_k}) - (\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}
$$

The second term is again treated as deferred correction.

The position of the auxiliary points are obtained by projecting $$\textbf{r}_k - \textbf{r}_{\mathrm{P}}$$ and $$\textbf{r}_{\mathrm{N}_k} - \textbf{r}_k$$ onto the face normal $$\textbf{n}$$. The magnitude of the shorter projection is recorded as $$\beta$$:

$$
\beta = \mathrm{min}\left((\textbf{r}_k - \textbf{r}_{\mathrm{P}}) \cdot \textbf{n}, (\textbf{r}_{\mathrm{N}_k} - \textbf{r}_k) \cdot \textbf{n}\right)
$$
...
$$
F^{\mathrm{d}}_k = \Gamma \left\{\frac{\phi_{\mathrm{N}_k}-\phi_{\mathrm{P}}}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|} + \frac{(\nabla \phi)_{\mathrm{N}_k} \cdot (\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{N}_k}) - (\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}\right\} S_k
$$

Collecting the coefficients:
$$
F^{\mathrm{d}}_k = -\left\{\frac{\Gamma S_k}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}\right\}\phi_{\mathrm{P}} + \left\{\frac{\Gamma S_k}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}\right\}\phi_{\mathrm{N}_k} + \frac{\Gamma S_k\{(\nabla \phi)_{\mathrm{N}_k} \cdot (\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{N}_k}) - (\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})\}}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}
$$

Its contribution to the linear system:
$$
\textbf{A}(i,i) \mathrel{+}= \frac{\Gamma S_k}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}
$$

$$
\textbf{A}(i,j) \mathrel{+}= -\frac{\Gamma S_k}{|\textbf{r}_{\mathrm{N}'_k} - \textbf{r}_{\mathrm{P'}}|}
$$
...
$$
\frac{\partial\phi_k}{\partial \textbf{n}} \approx \frac{\phi_k-\phi_{\mathrm{P}}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P}}|}
$$

Once again, $$\textbf{r}_k - \textbf{r}_{\mathrm{P}}$$ is likely not parallel to $$\textbf{n}$$:
...
$$
\frac{\partial\phi_k}{\partial \textbf{n}} \approx \frac{\phi_k-\phi_{\mathrm{P'}}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P}'}|} = \frac{\phi_k-\phi_{\mathrm{P}}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|} - \frac{(\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}
$$
...
$$
F^{\mathrm{d}}_k = \Gamma \left\{\frac{\phi_k-\phi_{\mathrm{P}}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|} - \frac{(\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}\right\} S_k
$$

Collecting the coefficients:
$$
F^{\mathrm{d}}_k = -\left\{\frac{\Gamma S_k}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}\right\}\phi_{\mathrm{P}} + \left\{\frac{\Gamma S_k}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}\right\}\phi_k - \frac{\Gamma S_k\{(\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})\}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}
$$

Its contribution to the linear system:
$$
\textbf{A}(i,i) \mathrel{+}= \frac{\Gamma S_k}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}
$$

$$
\textbf{b}(i) \mathrel{+}= \left\{\frac{\Gamma S_k}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}\right\}\phi_k - \frac{\Gamma S_k\{(\nabla \phi)_{\mathrm{P}} \cdot (\textbf{r}_{\mathrm{P'}} - \textbf{r}_{\mathrm{P}})\}}{|\textbf{r}_k - \textbf{r}_{\mathrm{P'}}|}
$$
</div>
